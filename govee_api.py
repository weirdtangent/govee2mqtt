...
class GoveeAPI(object):
    ...
    def reset_api_call_count(self):
        ...
        self.logger.info(f'Reset api call count for new day')
    ...
    def get_device_list(self):
        ...
        try:
            ...
            self.logger.error(f'ERROR GETTING DEVICE LIST DATA {r.content}')
            return {}
    ...
    def get_device(self, device_id, sku):
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                return {}
            data = r.json()
            self.logger.debug(f'GOT REFRESH ON ({device_id})')
        except RequestException as err:
            self.logger.error(f'REQUEST PROBLEM, RESTING FOR 10 SEC: {type(err).__name__} - {err=}')
            time.sleep(10)
        except Exception as err:
            self.logger.error(f'ERROR GETTING DEVICE DATA {r.content}')
            return {}
