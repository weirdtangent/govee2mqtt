...
class ServiceMixin:
    ...
    def publish_service_discovery(self):
        ...
        self.mqtt_safe_publish(
            topic=self.get_discovery_topic('number', f"{self.service_slug}_device_refresh"),
            payload=json.dumps({
                "name": f"{self.service_name} Device Refresh Interval",
                "uniq_id": f"{self.service_slug}_device_refresh",
                "stat_t": self.get_state_topic('service', 'service', 'device_refresh'),
                "json_attr_t": self.get_attribute_topic('service','service','device_refresh','attributes'),
    ...
    def publish_service_state(self):
        service = {
            ...
            "api_calls": {
                ...
                "last_api_call": self.goveec.get_last_call_date(),
            },
